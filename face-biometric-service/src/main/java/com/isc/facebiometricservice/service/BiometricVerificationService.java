...
public class BiometricVerificationService {
    ...
    public VerificationResponse verify(VerificationRequest request) {
        ...
        if (!"EMBEDDING".equalsIgnoreCase(request.capture().type())) {
            return invalid(request.requestId(), request.referenceId(), "EMBEDDING", "CAPTURE_TYPE_NOT_SUPPORTED", "The requested capture type is not supported.");
        }
        if (request.capture().embedding() == null || request.capture().embedding().size() != properties.dimension()) {
            return invalid(request.requestId(), request.referenceId(), "EMBEDDING", "EMBEDDING_DIMENSION_MISMATCH", "The embedding dimension does not match the configured verification model.");
        }
        ...
        if (properties.normalizedRequired() && !normalized) {
            return invalid(request.requestId(), request.referenceId(), "EMBEDDING", "NORMALIZED_EMBEDDING_REQUIRED", "A normalized embedding is required.");
        }
        ...
        if (!properties.modelId().equals(modelId) || !properties.modelVersion().equals(modelVersion)) {
            return invalid(request.requestId(), request.referenceId(), "EMBEDDING", "MODEL_MISMATCH", "The supplied embedding was generated by an incompatible biometric model.");
        }

        var reference = repository.find(request.referenceId(), modelId, modelVersion);
        if (reference.isEmpty()) {
            return outcome(request.requestId(), request.referenceId(), "EMBEDDING", VerificationStatus.INCONCLUSIVE,
                    "REFERENCE_NOT_FOUND", "No compatible biometric reference embedding is enrolled for this reference.",
                    null, null, null, null, null, List.of("REFERENCE_NOT_FOUND"));
        }
        ...
        try {
            ...
            return outcome(request.requestId(), request.referenceId(), "EMBEDDING", result, code, message,
                    similarity, properties.threshold(), null, null, null,
                    List.of(code));
        } catch (IllegalArgumentException ex) {
            return invalid(request.requestId(), request.referenceId(), "EMBEDDING", "INVALID_EMBEDDING", "The supplied embedding could not be verified.");
        }
    }

    private VerificationResponse invalid(String requestId, String referenceId, String method, String code, String message) {
        return outcome(requestId, referenceId, method, VerificationStatus.INVALID_REQUEST, code, message,
                null, properties.threshold(), null, null, null, List.of(code));
    }
    ...
    private VerificationResponse outcome(String requestId, String referenceId, String method,
        ...
        int httpStatus = switch (result) {
            ...
            case PROCESSING_ERROR -> 500;
            default -> 200;
        };
        return new VerificationResponse(requestId, referenceId, method, result, code, message, httpStatus,
                similarity, threshold, model == null ? metadata() : model,
