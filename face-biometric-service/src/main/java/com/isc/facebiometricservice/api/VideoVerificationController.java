...
public class VideoVerificationController {
    ...
    public VideoVerificationController(VideoVerificationProperties properties,
                                       BiometricProperties biometricProperties,
                                       FaceMatcher matcher,
                                       VideoClipDecoder decoder,
                                       VideoVerificationEngine engine,
                                       @Value("${biometric.capture-method:LIVE_STREAM}") String captureMethod) {
    ...
    public VerificationResponse verify(@RequestParam(required = false) String requestId,
        ...
        String selectedMethod = captureMethod == null || captureMethod.isBlank()
                ? configuredCaptureMethod : captureMethod.toUpperCase();
        log.info("[VIDEO][VERIFY_RECEIVED] requestId={} referenceId={} captureMethod={} bytes={}",
                id, referenceId, selectedMethod, clip == null ? 0 : clip.getSize());
    ...
    private String reason(String value) {
        return switch (value == null ? "" : value) {
            case "VIDEO_TOO_LONG", "VIDEO_TOO_LARGE", "NO_FACE", "MULTIPLE_FACES", "FACE_TOO_SMALL",
                 "LIVENESS_FAILED", "INVALID_VIDEO", "LOW_LIGHT", "BLUR", "BAD_POSE", "MODEL_ERROR",
                 "REFERENCE_NOT_FOUND", "LOW_QUALITY", "SIMILARITY_BELOW_THRESHOLD" -> value;
            default -> "INVALID_VIDEO";
        };
    }
}
