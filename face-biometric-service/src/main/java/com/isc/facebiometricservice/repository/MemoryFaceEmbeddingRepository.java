...
public class MemoryFaceEmbeddingRepository implements FaceEmbeddingRepository {
 ...
 @Override public Optional<FaceEmbedding> findByReferenceId(String userId){String prefix=userId+"|";return store.entrySet().stream().filter(e->e.getKey().startsWith(prefix)).map(java.util.Map.Entry::getValue).findFirst();}
}
