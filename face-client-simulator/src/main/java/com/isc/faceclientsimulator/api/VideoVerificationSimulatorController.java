...
public class VideoVerificationSimulatorController {
 ...
 public Object verify(@RequestParam String referenceId,@RequestParam(defaultValue = "FULL_CLIP") String captureMethod,@RequestPart("clip") MultipartFile clip)throws Exception{String requestId=UUID.randomUUID().toString();log.info("Simulator video verification request: requestId={}, referenceId={}, captureMethod={}, filename={}, contentType={}, bytes={}",requestId,referenceId,captureMethod,clip.getOriginalFilename(),clip.getContentType(),clip.getSize());return client.verify(requestId,referenceId,captureMethod,clip.getBytes(),clip.getOriginalFilename()==null?"clip.webm":clip.getOriginalFilename());}
}
