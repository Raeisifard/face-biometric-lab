...
public class BiometricPolicyServerClient {
    ...
    public Object createSession(String referenceId, String profile, String requestedMethod) {
        return rest.post().uri(uri -> uri.path("/api/v1/biometric/policy/sessions").queryParam("referenceId", referenceId).queryParamIfPresent("profile", java.util.Optional.ofNullable(profile)).queryParamIfPresent("requestedMethod", java.util.Optional.ofNullable(requestedMethod)).build()).retrieve().body(Object.class);
    }
    public Object createSession(String referenceId, String profile) { return createSession(referenceId, profile, null); }
