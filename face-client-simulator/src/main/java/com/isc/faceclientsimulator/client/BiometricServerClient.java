...
public class BiometricServerClient {
    ...
    public BiometricServerClient(RestClient restClient) {
    ...
    public com.isc.faceclientsimulator.api.SimulatorController.ServerStatusResponse health() {
        try {
            ...
            return new com.isc.faceclientsimulator.api.SimulatorController.ServerStatusResponse(
                    true,
                    response != null ? response.status() : "UNKNOWN",
                    "Biometric service reachable");
        } catch (Exception ex) {
    ...
    public ServerEnrollResponse enroll(String userId, FaceEmbedding e) {
        log.info("[SIM->BIOMETRIC] POST /api/v1/biometric/enroll-embedding user={} model={} version={}",
                userId, e.modelId(), e.modelVersion());
        var result = restClient.post()
                .uri("/api/v1/biometric/enroll-embedding")
                .body(new ServerEmbeddingPayload(userId, e.values(), e.dimension(), e.modelId(), e.modelVersion(), e.normalized()))
                .retrieve()
                .body(ServerEnrollResponse.class);
        log.info("[SIM<-BIOMETRIC] enroll response user={} enrolled={}", userId, result != null ? result.enrolled() : null);
        return result;
    }

    public ServerVerifyResponse verify(String userId, FaceEmbedding e) {
        log.info("[SIM->BIOMETRIC] POST /api/v1/biometric/verify-embedding user={} model={} version={}",
                userId, e.modelId(), e.modelVersion());
        var result = restClient.post()
                .uri("/api/v1/biometric/verify-embedding")
                .body(new ServerEmbeddingPayload(userId, e.values(), e.dimension(), e.modelId(), e.modelVersion(), e.normalized()))
                .retrieve()
                .onStatus(status -> status.isError(), (request, errorResponse) -> {})
                .body(ServerVerifyResponse.class);
        log.info("[SIM<-BIOMETRIC] verify response user={} result={} code={} similarity={}",
                userId,
                result != null ? result.result() : null,
                result != null ? result.code() : null,
                result != null ? result.similarity() : null);
        return result;
    }

    public ServerEnrollResponse enrollClient(String userId, FaceEmbedding e) {
        return enroll(userId, e);
    }

    public ServerVerifyResponse verifyClient(String userId, FaceEmbedding e) {
        return verify(userId, e);
    }
