...
public class ArcFaceOnnxEmbeddingModel {
    ...
    public ArcFaceOnnxEmbeddingModel(SimulatorProperties properties, L2Normalizer normalizer) {
        ...
        this.onnx = new OnnxSession(properties.recognitionModelPath());
    }
    ...
    public FaceEmbedding generate(Mat frame, FaceDetectionResult detection) {
        ...
        try {
            ...
            if (raw.length != 512) throw new IllegalStateException("ArcFace model output must be 512-D, actual=" + raw.length);
    ...
    private float[] toArcFaceTensor(Mat image) {
        ...
        f.get(0,0,hwc);
        f.release();
        float[] chw = new float[1*3*112*112];
        int plane = 112*112;
        for (int y=0;y<112;y++) {
            for (int x=0;x<112;x++) {
                int p=(y*112+x)*3;
