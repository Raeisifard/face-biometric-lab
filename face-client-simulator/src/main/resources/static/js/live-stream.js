(function () {
  ...
  function livePanel() {
    ...
    panel.innerHTML = '<div class="section-heading"><div><p class="eyebrow">METHOD 1</p><h2>Live stream</h2></div><span class="state-pill live-state-pill">Ready</span></div>' +
      ...
      '<div class="live-summary"><div><span>Server state</span><strong class="live-state">IDLE</strong></div><div><span>Frames</span><strong class="live-frames">0</strong></div><div><span>Detected faces</span><strong class="live-detected">0</strong></div><div><span>Latency</span><strong class="live-latency">-</strong></div><div><span>Upload</span><strong class="live-bytes">0 B</strong></div></div>' +
  ...
  async function sendFrame(canvas, ui) {
    ...
    try {
      ...
      if (ui.detected) ui.detected.textContent = result.detectedFaces ?? 0;
      ui.decoded.textContent = result.frameCount ?? state.frames; ui.recognition.textContent = result.feedbackCode === 'GOOD_FRAME' ? result.frameCount : ui.recognition.textContent;
  ...
  function applyVerificationResult(ui, result, roundTripMs) {
    ...
    ui.decoded.textContent = metrics.decodedFrames ?? 0; ui.recognition.textContent = metrics.recognitionFrames ?? 0; ui.processing.textContent = metrics.processingTimeMs != null ? metrics.processingTimeMs + ' ms' : '-'; ui.roundTrip.textContent = roundTripMs + ' ms';
    ...
    ui.pipeline.forEach(step => {
      const hasFailureForStep = (result.reasonCodes || []).some(code => code !== 'MATCH' && code.includes(step.dataset.step.toUpperCase()));
      step.className = hasFailureForStep ? 'failed' : terminal ? 'done' : steps.includes(step.dataset.step) ? 'done' : 'pending';
    });
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    const panel = livePanel(); const ui = { start: panel.querySelector('.live-start'), stop: panel.querySelector('.live-stop'), continue: panel.querySelector('.live-continue'), fps: panel.querySelector('.live-fps'), serverState: panel.querySelector('.live-state'), pill: panel.querySelector('.live-state-pill'), feedback: panel.querySelector('.live-feedback'), reason: panel.querySelector('.live-reason'), frames: panel.querySelector('.live-frames'), detected: panel.querySelector('.live-detected'), bytes: panel.querySelector('.live-bytes'), latency: panel.querySelector('.live-latency'), events: panel.querySelector('.live-events'), final: panel.querySelector('.final-result'), finalStatus: panel.querySelector('.final-status'), finalReason: panel.querySelector('.final-reason'), status: panel.querySelector('.live-result-status'), code: panel.querySelector('.live-code'), message: panel.querySelector('.live-message'), similarity: panel.querySelector('.live-similarity'), liveness: panel.querySelector('.live-liveness'), decoded: panel.querySelector('.live-decoded'), recognition: panel.querySelector('.live-recognition'), processing: panel.querySelector('.live-processing'), roundTrip: panel.querySelector('.live-round-trip'), reasons: panel.querySelector('.live-reasons'), pipeline: panel.querySelectorAll('.live-pipeline span') };
    try {
